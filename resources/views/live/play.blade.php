...
<body class="bg-slate-900 text-white min-h-screen font-sans overflow-hidden flex flex-col">
    ...
    <div class="fixed top-0 w-full bg-slate-800/90 backdrop-blur border-b border-slate-700 p-4 z-50 shadow-lg">
        <div class="max-w-5xl mx-auto flex justify-between items-center">
            <div class="flex items-center gap-3 w-1/3">
                ...
                <div><div class="text-[10px] text-blue-400 font-bold uppercase">HOST</div><div class="text-2xl font-bold font-mono" id="score-host">{{ $room->host_score }}</div></div>
            </div>
            <div class="text-center w-1/3"><div class="text-white font-bold text-sm animate-pulse" id="status-text">{{ $room->status == 'waiting' ? 'MENUNGGU...' : ($room->status == 'playing' ? 'LIVE' : 'SELESAI') }}</div></div>
            <div class="flex items-center gap-3 w-1/3 justify-end text-right">
                <div><div class="text-[10px] text-red-400 font-bold uppercase">LAWAN</div><div class="text-2xl font-bold font-mono" id="score-challenger">{{ $room->challenger_score }}</div></div>
                <div class="w-12 h-12 rounded-full bg-red-600 flex items-center justify-center text-xl font-bold border-2 border-white shadow-lg"><span id="p2-avatar">{{ $room->challenger ? substr($room->challenger->name, 0, 1) : '?' }}</span></div>
            </div>
        </div>
    </div>
    ...
    <div class="flex-grow flex items-center justify-center pt-24 pb-8 px-4">
        ...
        <div id="game-screen" class="{{ $room->status == 'playing' ? '' : 'hidden' }} w-full max-w-3xl">
            <div class="bg-slate-800 p-6 md:p-10 rounded-3xl border border-slate-700 shadow-2xl relative overflow-hidden">
                <div class="flex justify-between items-start mb-6">
                    <span class="bg-blue-600/20 text-blue-300 px-3 py-1 rounded-lg text-xs font-bold border border-blue-500/30">SOAL <span id="q-number">1</span></span>
                    <div class="text-center"><div class="text-xs text-slate-400 mb-1">WAKTU</div><span class="text-3xl font-mono font-bold text-yellow-400" id="timer">{{ $room->duration }}</span></div>
                </div>
                
                <div class="min-h-[100px] flex flex-col items-center justify-center text-center mb-8">
                    <h2 id="q-text" class="text-xl md:text-3xl font-bold leading-relaxed text-white">Memuat...</h2>
                </div>
                
                <div id="options" class="grid gap-3"></div>
                
                <button id="btn-confirm" onclick="submitComplexAnswer()" disabled class="hidden w-full mt-6 bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold transition disabled:opacity-50 disabled:cursor-not-allowed">
                    JAWAB SEKARANG
                </button>
                ...
                <div id="feedback-area" class="hidden mt-6 p-4 bg-slate-900 rounded-xl border-l-4 border-yellow-500 animate-slide-up relative z-20 shadow-inner">
                    ...
                    <div class="mt-2 text-xs text-gray-500 text-right animate-pulse">Lanjut otomatis dalam 3 detik...</div>
                </div>
            </div>
        </div>
        ...
        <div id="result-screen" class="{{ $room->status == 'finished' ? '' : 'hidden' }} text-center max-w-lg w-full bg-slate-800 p-10 rounded-3xl border border-slate-600 shadow-2xl">
            ...
            <div class="bg-slate-900/50 p-4 rounded-xl mb-6 grid grid-cols-2 gap-4">
                <div><div class="text-xs text-slate-500">Skor Anda</div><div class="text-2xl font-mono font-bold text-white" id="final-my-score">0</div></div>
                <div><div class="text-xs text-slate-500">Skor Lawan</div><div class="text-2xl font-mono font-bold text-white" id="final-enemy-score">0</div></div>
            </div>
            <a href="{{ route('menu') }}" class="bg-blue-600 hover:bg-blue-500 px-8 py-3 rounded-xl font-bold text-white transition block">Kembali ke Menu</a>
        </div>
    </div>
    ...
    <script>
        ...
        function loadQuestion() {
            if (currentIdx >= questions.length) { finishGameTrigger(); return; }
            ...
            // --- D. MATCHING ---
                ...
                displayOptions.forEach(opt => {
                    ...
                    selectHtml += `</select>`;
                    row.innerHTML = `<div class="font-bold mb-1 border-b border-gray-700 pb-1">${opt.option_text}</div> ${selectHtml}`;
        ...
        function submitSingle(opt, btn) {
            ...
            isAnswered = true;
            clearInterval(timerInterval);
            if (opt.is_correct) { btn.classList.add('correct-answer'); sendScore(100 + timeLeft); }
        ...
        function submitComplexAnswer() {
            ...
            if (isCorrect) sendScore(150 + timeLeft);
            finishTurn();
        }
        ...
        function showResultScreen(data) {
            ...
            if (data.winner_id == myId) { title.innerText = "🏆 KAMU MENANG!"; title.className = "text-4xl font-extrabold mb-2 text-green-400"; }
            else if (data.winner_id === null) { title.innerText = "🤝 SERI!"; title.className = "text-4xl font-extrabold mb-2 text-blue-400"; }
            else { title.innerText = "💀 KAMU KALAH"; title.className = "text-4xl font-extrabold mb-2 text-red-500"; }
        }
